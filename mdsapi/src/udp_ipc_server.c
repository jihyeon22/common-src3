...
// msg loop
static void _svr_msg_loop(void* arg)
{
...
void udp_ipc_server_start(int port_num, int (*msg_recv_proc)(const unsigned char* recv_msg, const int recv_msg_len, unsigned char* resp_msg, int* resp_msg_len))
{
    ...
    if(pthread_create(&g_mgr_msg_loop[msg_loop_idx].thread_var_svr_msg_loop, &attr, (void *)_svr_msg_loop, (void*) &g_mgr_msg_loop[msg_loop_idx].loop_arg) != 0) {
